...
export class AuthHandler {
  ...
  async handlePendingRequests(ctx: Context) {
    ...
    if (!adminId || !this.authService.isAdmin(adminId)) {
    ...
    try {
      ...
      logger.info('Pending requests viewed', { adminId, count: requests.length });
    } catch (error: any) {
  ...
  async handleApprove(ctx: Context, userIdToApprove: number) {
    ...
    try {
      ...
      const success = this.authService.approveUser(userIdToApprove, adminId);
      ...
      if (success) {
        ...
        logger.info('User approved', { userId: userIdToApprove, adminId });
      } else {
  ...
  async handleReject(ctx: Context, userIdToReject: number) {
    ...
    try {
      ...
      const success = this.authService.rejectUser(userIdToReject, adminId);
      ...
      if (success) {
        ...
        logger.info('User rejected', { userId: userIdToReject, adminId });
      } else {
  ...
  async handleRevokeAccess(ctx: Context, userIdToRevoke: number) {
    ...
    try {
      const success = this.authService.revokeAccess(userIdToRevoke, adminId);
      ...
      if (success) {
        ...
        logger.info('Access revoked', { userId: userIdToRevoke, adminId });
      } else {
  ...
  async handleApprovedUsers(ctx: Context) {
    ...
    try {
      ...
      logger.info('Approved users viewed', { adminId, count: users.length });
    } catch (error: any) {
...
<b>Режим:</b> ${modeNames[stats.authMode] || stats.authMode}
...
/approved - список одобренных
      ...
      logger.info('Auth stats viewed', { adminId });
    } catch (error: any) {
